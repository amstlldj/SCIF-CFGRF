...
def set_seed(seed):
    ...
    torch.cuda.manual_seed(seed)  # 设置单个GPU的随机种子
    torch.cuda.manual_seed_all(seed)  # 设置所有GPU的随机种子
    torch.backends.cudnn.deterministic = True  # 使得卷积运算的结果具有确定性
    torch.backends.cudnn.benchmark = False  # 关闭卷积的优化，防止某些硬件上使用非确定性算法
